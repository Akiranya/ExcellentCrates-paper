...
public class DataHandler extends AbstractUserDataHandler<ExcellentCrates, CrateUser> {
    ...
    public static @NotNull DataHandler getInstance(@NotNull ExcellentCrates plugin) throws SQLException {
        if (instance == null) {
    ...
    @Override
    protected @NotNull List<SQLColumn> getExtraColumns() {
    ...
    @Override
    protected @NotNull List<SQLValue> getSaveColumns(@NotNull CrateUser user) {
        return Arrays.asList(
    ...
    @Override
    protected @NotNull Function<ResultSet, CrateUser> getFunctionToUser() {
